                ...
                List {
                    ForEach(0..<vm.viewBestByDateItemList.count, id: \.self) { index in
                        HStack(spacing: .zero) {
                            ...
                            Button(
                                // 通知ボタン押下で通知可否状態を変更する
                                action: { vm.viewBestByDateItemList[index].notifyFlag.toggle() }
                            ){
                                // 通知可否の状態に応じて色を切り替える
                                Text("通知")
                                    .foregroundColor(vm.viewBestByDateItemList[index].notifyFlag ? .blue : .gray)
                                    .padding(2)
                                    .overlay(
                                        RoundedRectangle(cornerRadius: 5)
                                            .stroke(vm.viewBestByDateItemList[index].notifyFlag ? .blue : .gray, lineWidth: 2)
                                    )
                            }
